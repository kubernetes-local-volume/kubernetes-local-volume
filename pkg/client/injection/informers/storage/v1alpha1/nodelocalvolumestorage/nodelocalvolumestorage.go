...
package nodelocalvolumestorage
...
func withInformer(ctx context.Context) (context.Context, controller.Informer) {
	f := factory.Get(ctx)
	inf := f.Local().V1alpha1().NodeLocalVolumeStorages()
	return context.WithValue(ctx, Key{}, inf), inf.Informer()
}

// Get extracts the typed informer from the context.
func Get(ctx context.Context) v1alpha1.NodeLocalVolumeStorageInformer {
	untyped := ctx.Value(Key{})
	if untyped == nil {
		logging.FromContext(ctx).Panic(
			"Unable to fetch github.com/kubernetes-local-volume/kubernetes-local-volume/pkg/client/informers/externalversions/storage/v1alpha1.NodeLocalVolumeStorageInformer from context.")
	}
	return untyped.(v1alpha1.NodeLocalVolumeStorageInformer)
}
